...
#' @param verbose logical: if TRUE, provide additional progress output
#' @param local_dir_only logical: if TRUE, just return the local directory into which files from this data source would be saved
#' @param ... : additional parameters passed through to bb_handler_wget2
#'
#' @return the directory if local_dir_only is TRUE, otherwise TRUE on success
#'
#' @export
bb_handler_earthdata2 <- function(config,verbose=FALSE,local_dir_only=FALSE,...) {
    assert_that(is(config,"bb_config"))
    assert_that(nrow(bb_data_sources(config))==1)
    assert_that(is.flag(verbose),!is.na(verbose))
    assert_that(is.flag(local_dir_only),!is.na(local_dir_only))
    ...
    if (local_dir_only)
        return(bb_handler_wget2(config,verbose=verbose,local_dir_only=TRUE,...))
