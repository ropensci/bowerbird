...
bb_aadc_source <- function(metadata_id) {
    ...
    postproc <- list()
    csize <- tryCatch({
        s3x <- get_json(paste0("https://data.aad.gov.au/s3/api/bucket/datasets/science/", metadata_id))
    ...
    bb_source(name = md$data$entry_title,
              ...
              license = "CC-BY",
              method = list("bb_handler_aws_s3", bucket = "datasets", base_url = "public.services.aad.gov.au", region = "", prefix = paste0("science/", metadata_id), use_https = FALSE, bucketlist_json = paste0("http://data.aad.gov.au/s3/api/bucket/datasets/science/", metadata_id, "/")),
