...
test_that("bb_sync errors on a source that is missing required authentication info",{
    skip_on_cran()
    mysrc <- bb_example_sources("Sea Ice Trends and Climatologies from SMMR and SSM/I-SSMIS, Version 2")
...
test_that("bb_sync works with a sink() call in place",{
    ...
    myds <- bb_source(
        ...
        source_url="http://webdav.data.aad.gov.au/data/environmental/derived/readme.txt") ## just some file to download
    temp_root <- tempdir()
