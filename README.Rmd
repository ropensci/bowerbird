...
Bowerbird uses `wget` to recursively download files from data providers. `wget` is typically installed by default on Linux. For Windows you can try the `install_wget()` function or download it yourself (e.g. from https://eternallybored.org/misc/wget/current/wget.exe) and make sure it is on your path.
...
bb_save_config(cf,"/path/to/your/config.rds")

## load up a saved configuration
cf <- bb_config(file="/path/to/your/config.rds")
```

And once the configuration is defined or loaded, run the sync process:
```{r,eval=FALSE}
bb_sync(cf)
```
