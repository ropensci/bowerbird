...
## then the data source can be used as normal
cf <- bb_add(cf,mysrc)
```
...
by default wget considers itself to be a robot, and therefore won't recurse into areas of a site that are excluded to robots. This can cause problems with servers that exclude robots (accidentally or deliberately) from parts of their sites containing data that we want to retrieve. Setting \code{robots_off} to TRUE will add a "-e robots=off" flag, which instructs wget to behave as a human user, not a robot. See \url{https://www.gnu.org/software/wget/manual/wget.html#Robot-Exclusion} for more information about robot exclusion
...
1. If `wget` is not behaving as expected, try adding the `debug=TRUE` parameter to see additional diagnostic output.
