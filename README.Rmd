...
Bowerbird might be used in several different modes:
...
Bowerbird must then be told which data sources to synchronize. Use the `bb_source()` function to define a data source. Let's use a dataset of bike paths in Geelong as an example:
...
1. Remember that any `wget_global_flags` defined via `bb_config` will be applied to every data source in addition to their specific `method_flags`.
...
foreach (i=unique(cf$data_sources$name),.export=c("cf")) %dopar% {bb_sync(bb_subset(cf,cf$data_sources$name==i))}
```
